...
class API {
  Future<Map> getData(int index, int cpage) async {
    ...
    try {
      ...
      (index == 1)
          ? response = await get(
              Uri.parse('https://jaipurtimes.org/wp-json/wp/v2/posts/'))
          : response = await get(Uri.parse(
              'https://jaipurtimes.org/wp-json/wp/v2/posts/?categories=${S1[SECTIONS[index - 1]]}&&page=$cpage'));
      int page = 1;
      if (response.statusCode == 200) {
        ...
        if (index != 1) {
          ...
          Response response2 = await get(Uri.parse(
              'https://jaipurtimes.org/wp-json/wp/v2/categories/${S1[SECTIONS[index - 1]]}'));
